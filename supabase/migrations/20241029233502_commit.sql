alter table "public"."reviews" add column "beer" boolean;

alter table "public"."reviews" add column "food" boolean;

alter table "public"."reviews" add column "location" boolean;

alter table "public"."reviews" add column "music" boolean;

alter table "public"."reviews" add column "rating" integer not null;

alter table "public"."reviews" add column "service" boolean;

alter table "public"."reviews" add column "vibe" boolean;
...
create or replace view "public"."formatted_pubs" as  SELECT p.id,
    ...
    COALESCE((avg(r.rating))::double precision, (0)::double precision) AS rating,
    ...
    sum(DISTINCT
        CASE
            ...
            ELSE 0
        END) AS review_stars_one,
    sum(DISTINCT
        CASE
            WHEN (r.rating = 2) THEN 1
            ELSE 0
        END) AS review_stars_two,
    sum(DISTINCT
        CASE
            WHEN (r.rating = 3) THEN 1
            ELSE 0
        END) AS review_stars_three,
    sum(DISTINCT
        CASE
            WHEN (r.rating = 4) THEN 1
            ELSE 0
        END) AS review_stars_four,
    sum(DISTINCT
        CASE
            WHEN (r.rating = 5) THEN 1
            ELSE 0
        END) AS review_stars_five,
    p.description
   FROM ((((pubs p

...
SELECT
    ...
    (
        ...
        FROM
            (
                VALUES
                    (COALESCE(avg(r.vibe), 0 :: double precision)),
                    (COALESCE(avg(r.beer), 0 :: double precision)),
                    (COALESCE(avg(r.music), 0 :: double precision)),
                    (COALESCE(avg(r.service), 0 :: double precision)),
                    (COALESCE(avg(r.location), 0 :: double precision)),
                    (COALESCE(avg(r.food), 0 :: double precision))
            ) t(c)
    ) AS overall_reviews,

-- View: public.user_reviews
-- DROP VIEW public.user_reviews;
CREATE
OR REPLACE VIEW public.user_reviews AS
SELECT
    r.id,
    r.created_at,
    r.updated_at,
    r.pub_id,
    r.user_id,
    r.content,
    r.vibe,
    r.beer,
    r.music,
    r.service,
    r.location,
    r.food,
    r.editors_review,
    u.name AS user_name,
    count(
        CASE
            rh.is_helpful
            WHEN true THEN 1
            ELSE NULL :: integer
        END
    ) AS is_helpfuls,
    count(DISTINCT rh.*) AS total_helpfuls
FROM
    reviews r
    LEFT JOIN users_public u ON u.id = r.user_id
    LEFT JOIN review_helpfuls rh ON r.id = rh.review_id
GROUP BY
    r.id,
    u.id;

ALTER TABLE
    public.user_reviews OWNER TO postgres;

GRANT ALL ON TABLE public.user_reviews TO anon;

GRANT ALL ON TABLE public.user_reviews TO authenticated;

GRANT ALL ON TABLE public.user_reviews TO postgres;

GRANT ALL ON TABLE public.user_reviews TO service_role;